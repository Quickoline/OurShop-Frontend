...
import { getDiscoverPages } from '../config/discover';

export function DiscoverPage() {
  const { slug } = useParams<{ slug: string }>();
  const page = slug ? getDiscoverPages()[slug] : undefined;

  if (!page) {
    return (
      <main className="container mx-auto px-4 pt-36 pb-12 min-h-screen">
        <h1 className="text-3xl font-bold text-foreground">Page not found</h1>
        <p className="text-muted-foreground mt-2">This discover page doesn’t exist.</p>
        <div className="mt-6">
